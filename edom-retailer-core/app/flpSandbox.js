...
(function () {
    ...
    function mergeConfig(
        ...
        Object.keys(oActualConfigToMerge).forEach(function (sKey) {
            if (
                Object.prototype.toString.apply(oMutatedBaseConfig[sKey]) ===
                    '[object Object]' &&
                Object.prototype.toString.apply(oActualConfigToMerge[sKey]) ===
                    '[object Object]'
            ) {
                mergeConfig(
                    oMutatedBaseConfig[sKey],
                    oActualConfigToMerge[sKey],
                    false
                );
                return;
            }
    ...
    function adjustApplicationConfiguration(oConfig) {
        ...
        function fnGetApplicationKeys(oCfg) {
            ...
            for (sApplicationKey in oCfg.applications) {
                ...
                if (
                    oCfg.applications.hasOwnProperty(sApplicationKey) &&
                    sApplicationKey !== ''
                ) {
        ...
        function fnMakeTile(oApplication, iIdSuffix, sKey) {
            var sApplicationTitle =
                oApplication.title ||
                oApplication.additionalInformation
                    .replace('SAPUI5.Component=', '')
                    .split('.')
                    .pop();
            return {
        ...
        if (aApplicationKeys.length) {
            ...
            oLaunchPageAdapterConfig = jQuery.sap.getObject(
            ...
            aApplicationKeys.forEach(function (sApplicationKey, iSuffix) {
                oAutoGeneratedGroup.tiles.push(
                    fnMakeTile(
                        oConfig.applications[sApplicationKey],
                        iSuffix,
                        sApplicationKey
                    )
                );
            ...
            oNavTargetResolutionConfig = jQuery.sap.getObject(
    ...
    function applyDefaultApplicationConfig() {
        var sUrl =
                ...
                '../shells/sandbox/fioriSandboxConfig.json',
            oXHRResponse;

        jQuery.sap.log.info(
            'Mixing/Overwriting sandbox configuration from ' + sUrl + '.'
        );
        ...
        if (oXHRResponse.success) {
            jQuery.sap.log.debug(
                'Evaluating fiori launchpad sandbox config JSON: ' +
                    JSON.stringify(oXHRResponse.data)
            );
            ...
            mergeConfig(
                window['sap-ushell-config'],
                adaptInboundUrl(oXHRResponse.data),
                true
            );
        } else if (oXHRResponse.statusCode !== 404) {
            jQuery.sap.log.error(
                'Failed to load Fiori Launchpad Sandbox configuration from ' +
                    sUrl +
                    ': status: ' +
                    oXHRResponse.status +
                    '; error: ' +
        ...
        function adaptInboundUrl(data) {
            var oInbounds =
                    data.services.ClientSideTargetResolution.adapter.config
                        .inbounds,
                sBasePath = sap.ui.require
                    .toUrl('')
                    .substring(
                        0,
                        sap.ui.require.toUrl('').indexOf('resources')
                    ),
                sInboundUrl;
    ...
    function applyJsonApplicationConfig(sUrlPrefix) {
        var sUrl = jQuery.sap.endsWithIgnoreCase(sUrlPrefix, '.json')
                ? sUrlPrefix
                : sUrlPrefix + '.json',
            oXHRResponse;
    ...
    function getBootstrapScriptPath() {
        ...
        oBootstrapScript = window.document.getElementById(
    ...
    function evaluateCustomRenderer(sRenderer) {
        ...
        oApplications = jQuery.sap.getObject(
            'services.NavTargetResolution.adapter.config.applications',
            5,
            oSapShellConfig
        );
        ...
        if (
            typeof oApplications === 'object' &&
            oSapShellConfig.defaultRenderer !== 'fiorisandbox'
        ) {
    ...
    function bootstrap(fnCallback) {
        var aConfigFiles = jQuery.sap
                .getUriParameters()
                .get('sap-ushell-sandbox-config', true),
            sCustomRenderer = jQuery.sap
                .getUriParameters()
                .get('sap-ushell-sandbox-renderer'),
            i,
        ...
        jQuery.sap.registerModulePath(
            'sap.ushell.renderers.fiorisandbox',
            getBootstrapScriptPath() + '../renderers/fiorisandbox/'
        );
        ...
        var oUshellConfig = adjustApplicationConfiguration(
            window['sap-ushell-config']
        );
        ...
        if (oUshellConfig && oUshellConfig.modulePaths) {
            var oModules = Object.keys(oUshellConfig.modulePaths).reduce(
                function (result, sModulePath) {
                    result[sModulePath.replace(/\./g, '/')] =
                        oUshellConfig.modulePaths[sModulePath];
                    return result;
                },
                {}
            );
    ...
    sap.ushell.__sandbox__._adjustApplicationConfiguration =
        adjustApplicationConfiguration;
    sap.ushell.__sandbox__._applyJsonApplicationConfig =
        applyJsonApplicationConfig;
