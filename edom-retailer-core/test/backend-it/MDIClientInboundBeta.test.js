...
describe.skip('MDIClientInboundBeta it-test', () => {
    ...
    it(`should confirm a BP from S4 along with displayId in C4UF ${traceIssue2}`, async () => {
        // Scenario: S4 Create > Replicate
        // - Read MDIClient (receives create BP event, replicate to C4Uf)

        // mock MDI
        mdiMockServer.use(
            rest.patch(changeUrl, (req, res, ctx) => {
                return res(ctx.status(202));
            }),
            rest.get(logUrl, (req, res, ctx) => {
                return res(ctx.status(200), ctx.json(mockMDI.logResponse1));
        ...
        expect(status).toBe(200);
        expect(data.mdiBookKeeping.status).toBe('sent');
        expect(data.mdiBookKeeping.displayIdStatus).toBe('setInC4UF');
        expect(data.mdiBookKeeping.pending).toBeFalsy();
        ...
        expect(data2.mdiBookKeeping.status).toBe('confirmed');
        expect(data2.mdiBookKeeping.displayIdStatus).toBe('set');

        console.log(mockMDI.logResponse1);
    });
    ...
    it(`should reject BP update and set bookkeeping status as alert with error message ${traceIssue1}`, async () => {
        ...
        expect(getRes.data.mdiBookKeeping.status).toBe('sent');
        expect(getRes.data.mdiBookKeeping.pending).toBeFalsy();
    ...
    it(`should emit message after replicating created BP on MDI to C4Uf ${traceIssue3061}`, async () => {
        mdiMockServer.use(
            rest.get(logUrl, (req, res, ctx) => {
                return res(ctx.status(200), ctx.json(mockMDI.logResponse1));
            })
    ...
    it(`should emit message after replicating created and updated BP on MDI to C4Uf ${traceIssue3061}`, async () => {
        mdiMockServer.use(
            rest.get(logUrl, (req, res, ctx) => {
                return res(ctx.status(200), ctx.json(mockMDI.logResponse1));
            })
        ...
        // create BP on MDI
        mockMDI.insert('createFromS4', mdiObj, mockMDI.logResponse1);
        mockMDI.insert('updateFromS4', mdiObj, mockMDI.logResponse1);
    ...
    it(`should emit message after replicating created BP in C4Uf and updated BP on MDI ${traceIssue3061}`, async () => {
        // mock MDI
        mdiMockServer.use(
            rest.post(changeUrl, (req, res, ctx) => {
                mockMDI.insert(
...
/*describe('MDIClientInboundBeta2 it-test', () => {
    // enabling mock feature flags
    cds.env.requires.featureFlags = {
        impl: 'test/backend-it/external/FeatureFlagsTestService',
    };
    ...
    it(`should create BP C4Uf to MDI without displayId and update displayId with S4 local Id after reading MDIClient ${traceIssue3}`, async () => {
        ...
        expect(getData2.mdiBookKeeping.displayIdStatus).toBe('setInC4UF');
    });
